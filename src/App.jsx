...
export default function NIGPAnalyzer() {
  ...
  const [fetchDateTo,setFetchDateTo]=useState("12/31/2025");
  ...
  const fetchSelectedEventRef=useRef(null);
  const FETCH_API_BASE = import.meta.env.VITE_FETCH_API_URL || FETCH_API_BASE_DEFAULT;

  // Auto-scroll event list to bottom (latest)
  useEffect(()=>{
    if(fetchListRef.current && fetchRunning){
      fetchListRef.current.scrollTop=fetchListRef.current.scrollHeight;
    }
  },[fetchEvents,fetchRunning]);
  ...
  const runFetchAgent=useCallback(()=>{
    setFetchEvents([]);
    ...
    const url=`${FETCH_API_BASE}/agent/run?state=${fetchState}&year=${fetchYear}&dateFrom=${encodeURIComponent(fetchDateFrom)}&dateTo=${encodeURIComponent(fetchDateTo)}`;
  ...
  },[fetchState,fetchYear,fetchDateFrom,fetchDateTo,FETCH_API_BASE]);
  ...
  const handleFetchAnalyze=useCallback(async()=>{
    ...
    try{
      const url=`${FETCH_API_BASE}/agent/download?file=${encodeURIComponent(fetchComplete.filePath)}`;
  ...
  },[fetchComplete,fetchState,fetchYear,FETCH_API_BASE,processFile]);
                        ...
                        {fetchComplete?.success&&(
                          <>
                            <div style={{padding:"9px 12px",borderBottom:`1px solid ${T.lineSoft}`,borderLeft:`2.5px solid ${T.brass}`,paddingLeft:"9.5px",background:`rgba(182,135,58,0.05)`}}>
                              ...
                              <button onClick={()=>window.open(`${FETCH_API_BASE}/agent/download?file=${encodeURIComponent(fetchComplete.filePath)}`,"_blank")} style={{background:`linear-gradient(135deg,${T.brass},${T.brassDeep})`,color:T.navy,border:"none",padding:"6px 14px",cursor:"pointer",fontFamily:display,fontSize:11,fontWeight:700}}>↓ Save CSV File</button>
                            </div>
