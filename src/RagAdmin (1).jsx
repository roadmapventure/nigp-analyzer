...
// ─── EDIT PANEL ───────────────────────────────────────────────────────────────
function EditPanel({ entry, onSave, onCancel, isSaving }) {
  ...
  return (
    <div style={{ width: 340, flexShrink: 0, background: "#0d1424", borderLeft: "1px solid #1e3050", display: "flex", flexDirection: "column", overflowY: "auto" }}>
      ...
      <div style={{ padding: "13px 20px", borderTop: "1px solid #1e3050", display: "flex", gap: 10, background: "#0a0f1a", flexShrink: 0 }}>
        ...
        <button
          style={{ flex: 2, background: "linear-gradient(135deg,#2d8cf0,#00d2b4)", border: "none", borderRadius: 7, padding: 10, fontSize: 12, fontWeight: 700, color: "#0a0f1a", cursor: "pointer", fontFamily: "inherit", opacity: (!form.title || !form.content || isSaving) ? 0.5 : 1, transition: "opacity 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}
          disabled={!form.title || !form.content || isSaving}
          onClick={() => onSave(form)}
        >{isSaving ? "⏳ Indexing…" : "Save Entry"}</button>
      </div>
    </div>
  );
}
...
export default function RagAdmin() {
  ...
  const saveEntry = async (entry) => {
    setIsSaving(true);
    try {
      // Strip internal-only flag before sending to API
      const { _isNew, ...payload } = entry;
      ...
      if (!res.ok) {
        showToast(data.error || "Save failed — check console", "⚠");
        setIsSaving(false);
        return;
      }

      // Use the id returned by Supabase (important for new entries)
      const saved = { ...entry, ...data.entry, _isNew: undefined };
      const source = `Added ${new Date().toLocaleDateString("en-US",{month:"short",day:"numeric"})}`;

      if (entry._isNew) {
        setEntries(prev => [{ ...saved, source }, ...prev]);
        showToast("Entry saved to knowledge base ✦ Indexed");
      } else {
        setEntries(prev => prev.map(e => e.id === entry.id ? { ...saved, source: e.source } : e));
        showToast("Entry updated ✦ Re-indexed");
      }
      setEditEntry(null);
    } catch (err) {
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden", minWidth: 0 }}>
          ...
          {activeNav === "knowledge" && (
            <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
              ...
              {/* Edit panel */}
              {editEntry && <EditPanel entry={editEntry} onSave={saveEntry} onCancel={() => setEditEntry(null)} isSaving={isSaving} />}
            </div>
          )}
