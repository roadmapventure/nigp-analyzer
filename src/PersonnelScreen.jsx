        ...
        <div style={{padding:"10px 15px",borderBottom:`1px solid ${T.line}`,display:"flex",alignItems:"baseline",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            {agentEntries.length>0&&(
              <button onClick={()=>{
                // Build export object with all training entries
                const exportData = {
                  agent: agent.name,
                  agent_id: agent.id,
                  exported_at: new Date().toISOString(),
                  total_entries: agentEntries.length,
                  entries: agentEntries.map(e=>({
                    id: e.id,
                    title: e.title,
                    category: e.category,
                    source_type: e.source_type||"user",
                ...
                a.href = url;
                a.download = `${agent.id}-training-export-${new Date().toISOString().slice(0,10)}.json`;
                a.click();
                URL.revokeObjectURL(url);
              }} style={{fontFamily:mono,fontSize:9,textTransform:"uppercase",letterSpacing:1,color:T.brassDeep,background:"transparent",border:`1px solid ${T.line}`,padding:"2px 8px",cursor:"pointer"}}>
                ↓ Export
              </button>
            )}
          </div>
